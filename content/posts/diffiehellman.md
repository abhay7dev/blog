...
> Modular Exponentiation is a mathematical operation involving exponents and the remainders in divison, also called the modulo.
...
5. When Alice recieves `b*` and Bob recieves `a*`, they both perform one further mathematical operation on these keys, to finally calculate the final, shared key `x`.
(TutorialsPoint, 2024)
